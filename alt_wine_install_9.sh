...
then
  ...
  cd /tmp
  wget https://dl.winehq.org/wine/source/7.x/wine-7.16.tar.xz
  tar -xf wine-7.16.tar.xz
  ...
  libGLU-devel \
  libGLw-devel \
  libXv-devel > alt_wine_install.log

  #all
  echo "[ Нужные утилиты ]"
  apt-get -y install \
  desktop-file-utils \
  perl-XML-Simple >> alt_wine_install.log
  ...
  i586-libXv-devel >> alt_wine_install.log

  echo "[ Устанавливаем среду разработки ]"
  apt-get -y install i586-gcc8 i586-gcc8-c++ i586-gcc8-fortran i586-glibc-core >> alt_wine_install.log
  apt-get -y install gcc8 gcc8-c++ gcc8-fortran glibc-core cmake >> alt_wine_install.log
  apt-get -y install m4 bison flex build-environment >> alt_wine_install.log
  mkdir -p build-win64 build-win32
  cd build-win64
  echo "[ Собираем Win64 окружение ]"
  PKG_CONFIG_PATH=/usr/lib64 ../configure --enable-win64 --prefix=/usr >> alt_wine_install.log
  make -j2 >> alt_wine_install.log
  cd ../build-win32
  echo "[ Собираем Win32 окружение ]"
  PKG_CONFIG_PATH=/usr/lib ../configure --with-wine64=../build-win64 --prefix=/usr >> alt_wine_install.log
  make -j2 >> alt_wine_install.log
  echo "[ Устанавливаем WINE ]"
  make install >> alt_wine_install.log
  cd ../build-win64
